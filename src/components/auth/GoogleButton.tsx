...
export const GoogleButton: React.FC<GoogleButtonProps> = ({
  ...
  return (
    <div className="w-full flex flex-col items-center">
      {/* Official Google Identity Services iframe container */}
      <div
        ref={containerRef}
        id="google-gis-button"
        className={`w-full flex justify-center ${gisRendered ? 'block' : 'hidden'}`}
      />

      {/* Styled CAD button fallback when GIS is not rendered or while loading */}
      {!gisRendered && (
        <button
          type="button"
          onClick={onClickFallback}
          disabled={isLoading}
          className="w-full py-3.5 px-4 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-slate-100 font-semibold text-sm border border-slate-700 hover:border-slate-600 hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-3 relative group overflow-hidden active:scale-[0.99] disabled:opacity-50"
        >
