      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-200">
        <div className="flex items-center gap-3">
          <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl p-0.5">
            ...
            <div className="px-3 font-black text-sm text-red-600 font-mono-data tracking-wide min-w-[200px] text-center">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                <div>
                  <div
                    className={`font-black text-xs ${
                      isToday ? 'text-red-600' : isSelected ? 'text-red-600' : 'text-slate-900'
                    }`}
                  >
                    {day.dayLabel}
                  </div>
                ...
                {isToday && (
                  <span className="px-2 py-0.5 rounded-full bg-red-50 border border-red-200 text-red-600 font-mono-data font-bold text-[10px] animate-pulse">
                    Hôm nay
                  </span>
                )}
              </div>
              ...
              {shift && config ? (
                <div className="my-2.5 space-y-2.5 flex-1">
                  {/* Shift Badge Banner */}
                  <div
                    className={`p-2 sm:p-2.5 rounded-xl border flex items-center justify-between gap-1 min-w-0 ${config.bgClass} ${config.borderClass} ${config.textClass}`}
                  >
                    <div className="flex items-center gap-1.5 font-bold text-xs min-w-0 flex-1">
                      <config.icon className="w-3.5 h-3.5 shrink-0" />
                      <span className="truncate" title={config.label}>
                        {config.label}
                      </span>
                    </div>

                    {shift.isOvertime && (
                      <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 border border-red-200 text-[9px] font-mono-data font-bold shrink-0">
                        Tăng Cường
                      </span>
                    )}
                  </div>
                  ...
                  {shift.shiftType !== 'off' ? (
                    <div className="space-y-1.5 text-xs">
                      ...
                      {shift.vehiclePlate && shift.vehiclePlate !== '-' && (
                        <div className="flex items-center gap-1.5 text-red-600 font-mono-data font-bold">
                      ...
                      {shift.teamMembers && shift.teamMembers.length > 0 && (
                        <div className="pt-1 border-t border-slate-100">
                          <div className="text-[10px] text-slate-500 font-mono-data flex items-center gap-1 mb-1">
                          ...
                          <div className="space-y-0.5">
                            {shift.teamMembers.slice(0, 2).map((m) => (
                              <div
                                key={m.name}
                                className="text-[10px] text-slate-700 truncate flex items-center justify-between"
                              >
                                <span className="truncate">{m.name}</span>
                                <span className="text-slate-400 text-[9px] font-mono-data">
                                  {m.role.slice(0, 8)}
                                </span>
                              </div>
                            ))}
                    ...
                    <div className="py-4 text-center text-slate-500 text-xs">
                      <p className="font-medium text-slate-600">Nghỉ ca dưỡng sức</p>
                      <p className="text-[10px] mt-1 italic">Không có lịch tác chiến</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="my-6 text-center text-slate-400 text-xs flex-1 flex flex-col items-center justify-center space-y-2">
                  <p className="text-[11px]">Chưa xếp lịch ca</p>
                  <button
                    onClick={() => onQuickRegisterDate(day.dateStr)}
                    className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-lg text-[11px] font-mono-data font-semibold flex items-center gap-1 transition-colors"
                  >
                ...
                {shift ? (
                  <>
                    <button
                      ...
                      className="flex-1 py-1 px-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 text-[11px] font-medium transition-colors flex items-center justify-center gap-1"
                    >
                    ...
                    {shift.shiftType !== 'off' && shift.status !== 'completed' && (
                      <button
                        ...
                        className="p-1 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 text-[11px] transition-colors"
                      >
                  ...
                  <button
                    onClick={() => onQuickRegisterDate(day.dateStr)}
                    className="w-full py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-medium"
                  >
                    Đăng Ký Ca
                  </button>
                )}
