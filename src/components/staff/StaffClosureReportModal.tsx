  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-2 sm:p-4 md:p-6 text-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded bg-slate-800 text-[11px] font-mono-data text-slate-300">
                  Nhiệm vụ {mission.id}
                </span>
              </div>
        ...
        <form id="closure-form" onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="space-y-3 bg-slate-900/60 border border-slate-800 rounded-xl p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-400 border border-cyan-500/40 rounded-lg text-xs font-semibold transition-colors"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {photos.map((photo) => (
                <div
                  key={photo.id}
                  className="group relative rounded-xl border border-slate-700 bg-slate-950 overflow-hidden shadow-md flex flex-col"
                >
                  ...
                  <div className="p-2 text-xs text-slate-300 font-medium truncate">
                    {photo.label}
                    <div className="text-[10px] font-mono-data text-slate-500">{photo.timestamp}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-4 bg-slate-900/60 border border-slate-800 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                  <span className="text-[11px] text-slate-400 font-mono-data uppercase">Sơ Cứu Tại Chỗ</span>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                    ...
                    <span className="text-lg font-black text-cyan-400 font-mono-data flex-1 text-center">
                      {casualtiesTreated}
                    </span>
                    <button
                      type="button"
                ...
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                  <span className="text-[11px] text-slate-400 font-mono-data uppercase">Chuyển Viện Cấp Cứu</span>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                    ...
                    <span className="text-lg font-black text-emerald-400 font-mono-data flex-1 text-center">
                      {casualtiesHospitalized}
                    </span>
                    <button
                      type="button"
              ...
              <div className="space-y-2 pt-1">
                <label className="flex items-center justify-between p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-200 cursor-pointer">
            ...
            <div className="space-y-4 bg-slate-900/60 border border-slate-800 rounded-xl p-4">
              ...
              {/* Action Summary Note */}
              <div>
                <label className="text-[11px] text-slate-400 font-mono-data block mb-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 focus:border-cyan-500 focus:outline-none leading-relaxed"
              ...
              <div className="space-y-1.5">
                <div className="text-[11px] text-slate-400 font-mono-data">Vật tư tiêu hao:</div>
                <div className="space-y-1">
                  {resources.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800/80 text-xs"
                    >
        ...
        <div className="p-4 bg-slate-900 border-t border-slate-800 flex items-center justify-end gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl border border-slate-700 bg-slate-800 text-slate-300 hover:text-white text-xs sm:text-sm font-semibold transition-all"
          >
            Đóng
          </button>
          ...
          <button
            type="submit"
            form="closure-form"
            className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs sm:text-sm font-bold shadow-lg shadow-emerald-950/50 border border-emerald-400/40 flex items-center gap-2 transition-all active:scale-95"
          >
